...
export function CoachSettingsClient({ profile }: { profile: { full_name: string; specialisation: string; bio: string; initial: string } }) {
        ...
        <div className="panel">
          ...
          <form action={action}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
              ...
              <button type="button" className="gf-btn gf-btn-secondary gf-btn-sm">Change photo</button>
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="panel">
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {DAYS.map((d, i) => (
                <button key={d} onClick={() => setAvail((s) => { const n = new Set(s); n.has(i) ? n.delete(i) : n.add(i); return n; })} className={`gf-chip${avail.has(i) ? ' active' : ''}`} style={{ cursor: 'pointer' }}>{d}</button>
              ))}
            </div>
          </div>
          <div className="panel">
