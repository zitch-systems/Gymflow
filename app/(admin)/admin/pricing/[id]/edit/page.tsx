...
export default async function EditPlan({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: plan } = await supabase
    .from('membership_plans').select('id, name, price, duration_days, duration_months, is_active, trainer_addon_enabled, trainer_addon_price')
  ...
  return (
    <>
      ...
      <div className="panel" style={{ maxWidth: 640 }}>
        <PlanForm plan={{ id: plan.id, name: plan.name, price: Number(plan.price), duration_days: plan.duration_days, duration_months: plan.duration_months, is_active: plan.is_active, trainer_addon_enabled: plan.trainer_addon_enabled, trainer_addon_price: Number(plan.trainer_addon_price ?? 0) }} />
      </div>
    </>
  );
}
