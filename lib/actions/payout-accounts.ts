...
// Add a payout account (up to MAX_ACCOUNTS). Verification is optional/best-
// effort. The first account a gym adds becomes active automatically.
export async function addPayoutAccount(_prev: PayoutState, formData: FormData): Promise<PayoutState> {
  ...
  try {
    ...
    // Best-effort verify — doesn't block saving.
    let resolvedName = account_name;
    let verified = false;
    if (process.env.PAYSTACK_SECRET_KEY) {
      const r = await resolveAccount(account_number, bank_code);
      if (r.ok) { resolvedName = r.accountName || account_name; verified = true; }
    }
...
// Switch which saved account is active. Self-service among the gym's own
// accounts — no platform approval.
...
// Remove a saved account. If it was active, the newest remaining account (if
// any) becomes active and the gyms row is re-synced.
export async function removePayoutAccount(_prev: PayoutState, formData: FormData): Promise<PayoutState> {
  ...
  try {
    ...
    if (account.is_active) {
      // Promote the most recently added remaining account, if any.
      const { data: next } = await supabase.from('gym_payout_accounts' as never)
        .select('id').eq('gym_id', gym.id).order('created_at', { ascending: false }).limit(1).maybeSingle();
