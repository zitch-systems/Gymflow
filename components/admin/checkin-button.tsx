...
export function CheckInButton({ memberId, checkedIn = false }: { memberId: string; checkedIn?: boolean }) {
  ...
  if (state.ok) {
    return (
      <span role="status" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, color: 'var(--gf-success)', fontFamily: 'var(--gf-font-display)', fontWeight: 700, fontSize: '0.8rem' }}>
  ...
  return (
    <form action={action} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {state.error && <span role="alert" style={{ color: 'var(--gf-danger)', fontSize: '0.74rem' }}>{state.error}</span>}
    </form>
  );
}
