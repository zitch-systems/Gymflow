...
export function ContactForm() {
  ...
  return (
    <form className="cform" action={action}>
      ...
      {state.error && <p className="full" style={{ display: 'flex', alignItems: 'center', gap: 7, color: 'var(--gf-danger)', fontSize: '0.84rem', margin: 0 }}><AlertCircle size={15} strokeWidth={2} /> {state.error}</p>}
