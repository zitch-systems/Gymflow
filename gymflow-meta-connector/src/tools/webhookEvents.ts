 ...
 * updating as expected; it is not a log of specific past deliveries. If
 * GymFlow wants true webhook-event history, that has to come from the
 * backend's own event log via a dedicated read-only ops endpoint —
 * deliberately out of scope here, since this connector does not touch the
 * production webhook.
 */
