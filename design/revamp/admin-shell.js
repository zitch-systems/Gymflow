...
(function () {
  ...
  const NAV = [
    ...
    { s: 'Operations', items: [
      { k:'staff',     label:'Staff',     icon:'user-cog',         href:'admin-staff.html' },
      { k:'facility',  label:'Facility',  icon:'dumbbell',         href:'admin-facility.html' },
  ...
  window.adminShell = function (active, topOpts) {
    ...
    if (!document.getElementById('gfUiJs')) { const us = document.createElement('script'); us.id = 'gfUiJs'; us.src = 'ui.js'; document.body.appendChild(us); }
    const tbtn = document.getElementById('admTheme');
    if (tbtn) {
      const ico = () => { const d = document.documentElement.getAttribute('data-theme') === 'dark'; tbtn.innerHTML = `<i data-lucide="${d ? 'sun' : 'moon'}"></i>`; if (window.lucide) lucide.createIcons(); };
      ico();
      tbtn.onclick = () => {
